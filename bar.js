...
function bar(svg, down, d, selector) {
    ...
    const bar = g.selectAll("g")
        ...
        .on("mouseover", over)
        .on("click", d => down(svg, d, odss_bar[d.data.name] != undefined ? odss_bar[d.data.name].color : "rgb(229,147,93)"));
...
function clicked(d){
    ...
    barrioName2 = d.data.name
    var req_sunbu = { ...req };
    req_sunbu.numero = 400;
    req_sunbu.respuesta = [2];
    req_sunbu.barrios = [barrioName2]
    escribirBarrio()
    postData(urlBack+'sunburst', req_sunbu).then(data => {
        data.name = "ODS";
...
function over(d) {
    ...
    if (d.parent.height!=3){
        return
    }



    var barrio_over = 'B'+barr_dict[d.data.name]
    barrioName = d.data.name
	...
	d3.select("#tooltip").html(tooltipHtml(d.value))
		.style("left", d3.event.layerX + "px")
		.style("top", d3.event.layerY + "px")
		.style("z-index", "1000");

    cambiar_de_comuna(barrio_over)
...
function down(svg_bar, d_param, color_ods) {
    ...
    enter.selectAll("rect")
        ...
        .attr("fill", d => odss_bar[d.data.name] != undefined ? odss_bar[d.data.name].color : odss_bar[d.children[0].data.name].color || "rgb(0,0,0)")
        .attr("width", d => {
            //
            //
            return x(d.value) - x(0);
...
function up(svg_bar, d_param) {
    ...
    enter.selectAll("rect")
    .attr("fill", d => odss_bar[d.data.name] != undefined ? odss_bar[d.data.name].color : "rgb(229,147,93)")
        .attr("fill-opacity", p => p === d_param ? 0 : null)
        .transition(transition2)
        .attr("width", d => {
            //
